...
生成“研究区内正常显示、研究区外淡化/透明化”的研究区范围图。
...
def compose_effect(
    ...
    *,
    ...
    # 区域外：转灰 + 泛白，形成“半透明蒙版”视觉
    gray = np.mean(rgb, axis=2, keepdims=True)
    outside = np.repeat(gray, 3, axis=2)
    outside = outside * (1.0 - outside_fade) + np.ones_like(outside) * outside_fade
...
def plot_study_area(
    *,
    ...
    ax.plot(per[:, 0], per[:, 1], color=boundary_color, lw=boundary_lw, alpha=0.85, solid_capstyle="round", zorder=4)
...
def main():
    ...
    p.add_argument("--tif", default=r'D:\Work\qyb\ResNet-18\YD_DEM.tif', help="GeoTIFF 遥感底图")
    p.add_argument("--out", default="DEM_area4.png", help="输出 PNG 路径")
    ...
    p.add_argument("--outside_fade", type=float, default=0.78, help="研究区外泛白程度，0-1，越大越白")
    p.add_argument("--boundary_color", default="#d9d28f", help="边界线颜色")
    ...
    p.add_argument("--crop_margin_left", type=float, default=0.06, help="左侧单独外扩比例")
    p.add_argument("--crop_margin_right", type=float, default=0.06, help="右侧单独外扩比例")
    p.add_argument("--crop_margin_top", type=float, default=0.03, help="上侧单独外扩比例")
    p.add_argument("--crop_margin_bottom", type=float, default=0.09, help="下侧单独外扩比例")
